...
description: Analyzes project requirements and recommends optimal teams of specialized agents. Use PROACTIVELY for complex multi-domain tasks requiring coordinated agent collaboration.
...
- **frontend-developer** - Framework-agnostic frontend engineer (React, Vue, Angular, Svelte, Solid) specializing in responsive design, component architecture, and modern frontend patterns with performance optimization and accessibility compliance.
- **product-designer** - Comprehensive product designer covering visual UI design and UX research. Handles design systems, prototyping, user flows, usability evaluation, and accessibility across all platforms.
- **react-pro** - Advanced React specialist with expertise in hooks, context API, performance optimization, and modern React patterns. For React-specific deep work; use frontend-developer for general frontend.
...
- **backend-developer** - Implementation-focused backend developer that writes production code, tests, and migrations. Counterpart to backend-architect.
- **full-stack-developer** - End-to-end vertical slice developer for tasks spanning frontend and backend. Use for features that touch both UI and server.
...
- **git-workflow** - Git operations specialist for branching strategies, conventional commits, PR workflows, conflict resolution, and release tagging.
...
- **documentation-expert** - Technical writing specialist creating READMEs, guides, tutorials, onboarding docs, architecture docs, and documentation systems.

### Orchestration Agents

- **context-manager** - Project context tracking and shared state management. Scans projects to compile structured context summaries for other agents.
- **performance-monitor** - Performance monitoring specialist for bottleneck identification, resource usage analysis, and optimization recommendations.
...
- **Role in Project:** Conduct comprehensive security-focused code review of the authentication system and existing application vulnerabilities
- **Justification:** Authentication introduces critical security vectors that must be professionally reviewed. This agent includes security analysis capabilities covering OWASP compliance and vulnerability assessment.
- **Key Contributions:** Security vulnerability report, authentication security validation, secure coding recommendations, code quality assessment of auth endpoints
...
3. **Phase 3:** `code-reviewer` - Conduct security review of implementation and overall application
4. **Phase 4:** `api-documenter` - Create comprehensive API documentation and update project docs

**Agent Coordination Strategy:**

- `backend-architect` provides implementation details to `code-reviewer` for review
- `code-reviewer` findings feed back to `backend-architect` for remediation
...
2. **Follow with `code-reviewer`** - Review the backend-architect's implementation  
