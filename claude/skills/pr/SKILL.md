...
Get the current branch and use it as the source branch:
...
Ask these questions using AskUserQuestion in a single round:

1. **PR type** — feat / fix / refactor / docs / chore
2. **Summary** — "Describe what this PR does." (expect free text via Other)
3. **Test plan** — "How should this be tested?" (expect free text via Other)
...
```markdown
## Summary
<bullet points from commit analysis + user summary>
...
If a PR exists, show its URL and ask whether to update it or abort.
...
Create the PR (add `--draft` if the user chose "Accept as draft" in Step 5):
